      ...
      >
        <n-scrollbar>
          <n-list class="category-list" :show-divider="false">
            ...
            <n-list-item 
              class="category-item"
              :class="{ active: toolStore.selectedCategoryId === 'all' }"
            ...
            <n-list-item 
              ...
              :key="category.id"
              class="category-item"
              :class="{ active: toolStore.selectedCategoryId === category.id }"
            >
        ...
        <div class="tools-header">
          <div class="tools-header-left">
            <n-input
              ...
              clearable
              class="tools-search"
              @update:value="toolStore.setSearchKeyword"
            >
              <template #prefix>
        ...
        <n-spin :show="toolStore.loading || categoryStore.loading">
          <div class="tools-cards">
            <n-grid :cols="4" :x-gap="16" :y-gap="16" responsive="screen">
              <n-grid-item v-for="tool in toolStore.displayTools" :key="tool.id">
            ...
            <div v-if="toolStore.displayTools.length === 0 && !toolStore.loading" class="empty-state-container">
...
const emptyDescription = computed(() => {
  if (toolStore.searchKeyword) {
    return '没有找到匹配的工具';
  }
  return toolStore.selectedCategoryId === 'all' ? '暂无工具' : '该分类下暂无工具';
});

// 处理分类选择 - 修改函数实现
function handleSelectCategory(categoryId: number | 'all') {
  console.log(`选择分类: ${categoryId}`);
  toolStore.selectCategory(categoryId);
  // 强制刷新数据
  if (categoryId === 'all') {
    toolStore.fetchAllTools();
  } else {
    toolStore.fetchToolsByCategory(categoryId);
  }
}
...
async function deleteCategory() {
  ...
  try {
    ...
    if (success) {
      if (toolStore.selectedCategoryId === categoryToDelete.value.id) {
        toolStore.selectCategory('all');
      }
...
watch(() => toolStore.needRefresh, (needRefresh) => {
  if (needRefresh) {
    ...
    // 直接触发handleSelectCategory函数，传入当前选中的分类ID
    handleSelectCategory(toolStore.selectedCategoryId);
  }
}, { immediate: true }); 
...
onMounted(async () => {
  ...
  // 直接调用handleSelectCategory而不是fetchAllTools
  // 这样确保选中状态和数据都会更新
  handleSelectCategory(toolStore.selectedCategoryId);
});
</script>
