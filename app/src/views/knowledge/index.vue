      ...
      <div class="knowledge-header">
        <div class="knowledge-header-left">
          <n-input
            ...
            @update:value="knowledgeStore.setSearchKeyword"
          >
            <template #prefix>
      ...
      <n-spin :show="knowledgeStore.loading || categoryStore.loading">
        <div class="knowledge-cards">
          <n-grid :cols="3" :x-gap="16" :y-gap="16" responsive="screen">
            <n-grid-item v-for="kb in knowledgeStore.displayKnowledgeBases" :key="kb.id">
          ...
          <!-- 空状态 -->
          <div v-if="knowledgeStore.displayKnowledgeBases.length === 0 && !knowledgeStore.loading" class="empty-state-container">
...
import { 
  ...
  NInput, NEmpty, NSpace, NSpin, NModal, NTag
} from 'naive-ui';
import { 
...
const emptyDescription = computed(() => {
  if (knowledgeStore.searchKeyword) {
    return '没有找到匹配的知识库';
  }
  return knowledgeStore.selectedCategoryId === 'all' ? '暂无知识库' : '该分类下暂无知识库';
});

// 处理分类选择
function handleSelectCategory(categoryId: string | 'all') {
  console.log(`选择分类: ${categoryId}`);
  knowledgeStore.selectCategory(categoryId);
  // 加载数据
  if (categoryId === 'all') {
    knowledgeStore.fetchAllKnowledgeBases();
  } else {
    knowledgeStore.fetchKnowledgeBasesByCategory(categoryId);
  }
}
...
watch(() => knowledgeStore.needRefresh, (needRefresh) => {
  if (needRefresh) {
    ...
    handleSelectCategory(knowledgeStore.selectedCategoryId);
  }
}, { immediate: true }); 
