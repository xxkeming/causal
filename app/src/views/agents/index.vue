<template>
  <div class="agents-view">
    <n-layout has-sider position="absolute">
      ...
      >
        <n-scrollbar>
          <n-list class="category-list" :show-divider="false">
            ...
            <n-list-item 
              class="category-item"
              :class="{ active: agentStore.selectedCategoryId === 'all' }"
            ...
            <n-list-item 
              ...
              :key="category.id"
              class="category-item"
              :class="{ active: agentStore.selectedCategoryId === category.id }"
            >
          ...
          <div class="agents-header">
            <div class="agents-header-left">
              <n-input
                ...
                clearable
                class="agents-search"
                @update:value="agentStore.setSearchKeyword"
              >
                <template #prefix>
          ...
          <n-spin :show="agentStore.loading || categoryStore.loading">
            <div class="agents-cards">
              <n-grid :cols="4" :x-gap="16" :y-gap="16" responsive="screen">
                <n-grid-item v-for="agent in agentStore.displayAgents" :key="agent.id">
                    ...
                    <template #footer>
                      <div class="agent-card-footer">
                        ...
                        <div class="agent-card-actions">
                          <n-space>
                            <n-button circle tertiary size="small" type="primary" title="启动会话" @click="startChat(agent)">
                              <template #icon>
              ...
              <!-- 修改空状态组件，使其垂直居中并移除创建按钮 -->
              <div v-if="agentStore.displayAgents.length === 0 && !agentStore.loading" class="empty-state-container">
      ...
      <agent-form-modal
        ...
        :default-category-id="agentStore.selectedCategoryId"
  ...
  async function handleSelectCategory(categoryId: string) {
    // agentStore.selectCategory(categoryId);
    await agentStore.fetchAgentsByCategory(categoryId);
  }
  ...
  async function deleteCategory() {
    ...
    try {
      ...
      if (success) {
        ...
        if (agentStore.selectedCategoryId === categoryId) {
          agentStore.selectCategory('all');
        }
  ...
  async function handleAgentFormSubmit(agent: Agent) {
    try {
      ...
      // 刷新智能体列表
      if (agentStore.selectedCategoryId === 'all') {
          await agentStore.fetchAllAgents();
        } else {
          await agentStore.fetchAgentsByCategory(agentStore.selectedCategoryId);
      }
    } catch (error) {
  ...
  /* 直接在组件中设置按钮和输入框样式 */
  :deep(.agents-search .n-input__border),
  :deep(.agents-search .n-input-wrapper) {
